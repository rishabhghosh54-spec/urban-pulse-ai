# ╔══════════════════════════════════════════════════════════════════════╗
# ║  URBAN PULSE AI  ·  Smart City Stress Command Center  ·  v2.0       ║
...
:root {
  ...
  --gb:     rgba(0,229,255,0.12);   /* glass border */
  --gs:     rgba(0,229,255,0.05);   /* glass shine  */
...
[data-testid="stAppViewContainer"]::after {
  ...
  background-image: linear-gradient(var(--gb) 1px,transparent 1px),linear-gradient(90deg,var(--gb) 1px,transparent 1px);
...
/* ── Sidebar ── */
[data-testid="stSidebar"]{background:var(--ink1)!important;border-right:1px solid var(--bdr)!important;}
...
.kc::after{
  ...
  background:linear-gradient(90deg,transparent,var(--cdim),transparent); opacity:0; transition:.25s;
}
.kc:hover::after{opacity:1;}
.kc-lbl{font-family:'DM Mono',monospace;font-size:.60rem;letter-spacing:2.5px;text-transform:uppercase;color:var(--tx3);margin-bottom:8px;}
.kc-val{font-family:'Barlow Condensed',sans-serif;font-size:2.5rem;font-weight:700;color:var(--cyan);line-height:1;}
...
/* ── Pulse dot ── */
@keyframes pulse-dot{0%,100%{opacity:1;box-shadow:0 0 6px currentColor;}50%{opacity:.3;box-shadow:none;}}
.dot{display:inline-block;width:8px;height:8px;border-radius:50%;animation:pulse-dot 2.2s ease-in-out infinite;}
...
.ir{
  ...
  background:var(--glass); border:1px solid var(--gb); border-radius:10px; margin-bottom:10px;
}
...
.itag{
  ...
  text-transform:uppercase; padding:2px 8px; border-radius:4px; margin-bottom:4px; display:inline-block;
}

/* ── Alert banner ── */
.ab{padding:14px 18px;border-radius:10px;margin-bottom:10px;display:flex;align-items:flex-start;gap:12px;font-size:.85rem;line-height:1.5;}

/* ── Slider overrides ── */
.stSlider>label{font-family:'DM Mono',monospace!important;font-size:.65rem!important;letter-spacing:2px!important;text-transform:uppercase!important;color:var(--tx2)!important;}
div[data-testid="stSlider"] [role="slider"]{background:var(--cyan)!important;border-color:var(--cyan)!important;box-shadow:0 0 10px var(--cglow)!important;}
...
/* ── Selectbox ── */
.stSelectbox label,.stMultiSelect label{font-family:'DM Mono',monospace!important;font-size:.62rem!important;letter-spacing:2px!important;text-transform:uppercase!important;color:var(--tx2)!important;}
[data-baseweb="select"]>div{background:var(--ink2)!important;border-color:var(--bdr)!important;color:var(--tx)!important;}
...
#  CONSTANTS
# ═══════════════════════════════════════════════════════════════════════
CY = "#00e5ff"; GR = "#00f5a0"; AM = "#ffb340"; RD = "#ff4060"
TX = "#deeeff"; D2 = "#8aaec8"; D3 = "#3d6278"
INK = "#070d14"; INK2 = "#0c1622"; BDR = "rgba(0,229,255,0.09)"
...
FMETA = {
    "traffic_volume":        {"ico":"🚦","lbl":"Traffic Volume",        "unit":"/100", "lo":0,   "hi":100,  "step":1.0,  "def":72},
    "aqi":                   {"ico":"💨","lbl":"Air Quality Index",      "unit":" AQI", "lo":10,  "hi":500,  "step":5.0,  "def":140},
    "noise_pollution":       {"ico":"🔊","lbl":"Noise Pollution",        "unit":" dB",  "lo":30,  "hi":100,  "step":1.0,  "def":70},
    "temperature":           {"ico":"🌡️","lbl":"Temperature",            "unit":" °C",  "lo":5,   "hi":48,   "step":0.5,  "def":29},
    "humidity":              {"ico":"💧","lbl":"Humidity",               "unit":"%",    "lo":10,  "hi":100,  "step":1.0,  "def":65},
    "pop_density":           {"ico":"👥","lbl":"Population Density",     "unit":"/km²", "lo":1000,"hi":40000,"step":500.0,"def":11000},
    "public_transport_usage":{"ico":"🚇","lbl":"Public Transport Usage", "unit":"/100", "lo":0,   "hi":100,  "step":1.0,  "def":60},
}
MODEL_OPTIONS = {
...
def stress_tier(s):
    try:
        v = float(s)
        if v < 35:  return GR, "LOW",      "rgba(0,245,160,0.12)",  "dg"
        if v < 65:  return AM, "MODERATE", "rgba(255,179,64,0.12)", "da"
        return RD, "HIGH",     "rgba(255,64,96,0.12)",  "dr"
    except Exception:
        return D2, "UNKNOWN", "rgba(100,100,100,.12)", "da"
...
def plot_base(title="", h=340, xl="", yl="", legend=False):
    return dict(
        ...
        title=dict(text=title, font=dict(family="Barlow Condensed, sans-serif", color=TX, size=15), x=0.01),
        margin=dict(l=14,r=14,t=46,b=14), height=h,
        xaxis=dict(gridcolor="rgba(0,229,255,0.06)",zerolinecolor="rgba(0,229,255,0.12)",
                   tickfont=dict(size=10), title=dict(text=xl,font=dict(size=10))),
        yaxis=dict(gridcolor="rgba(0,229,255,0.06)",zerolinecolor="rgba(0,229,255,0.12)",
                   tickfont=dict(size=10), title=dict(text=yl,font=dict(size=10))),
        showlegend=legend,
        legend=dict(font=dict(family="DM Mono",size=9,color=D2),bgcolor="rgba(0,0,0,0)",
                    orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1) if legend else {},
        hoverlabel=dict(bgcolor=INK2,font=dict(family="DM Mono",size=11,color=TX),bordercolor=CY),
    )
...
def train_model(name):
    ...
    try:
        ...
        sc   = StandardScaler()
        Xts  = sc.fit_transform(Xtr); Xes = sc.transform(Xte)
        ...
        return mdl, sc, {
            ...
            "imp":imp,"yte":yte,"yp":yp,
            "trn":len(Xtr),"tst":len(Xte),"ns":ns,
        }
...
with st.sidebar:
    ...
    <div style='padding:8px 0 24px;'>
      <div style='font-family:Barlow Condensed,sans-serif;font-size:1.55rem;
                  font-weight:800;letter-spacing:3px;color:{CY};text-transform:uppercase;line-height:1;'>
        URBAN PULSE
      </div>
      ...
      <div style='margin-top:12px;display:flex;align-items:center;gap:8px;'>
        ...
        <span style='font-family:DM Mono,monospace;font-size:.60rem;color:{GR};letter-spacing:1.5px;'>
          SYSTEM OPERATIONAL
        </span>
      </div>
    </div>
    ...
    model_choice = st.selectbox("🧠  Prediction Model", list(MODEL_OPTIONS.keys()), index=0)
    ...
    <div style='font-family:DM Mono,monospace;font-size:.58rem;letter-spacing:2px;color:{D3};
                text-transform:uppercase;margin-bottom:6px;'>Session</div>
...
r2pct = mtr["r2"]*100; conf = min(99.9, r2pct+3.2)
...
<div style='display:flex;align-items:center;gap:20px;padding:22px 0 18px;
  ...
  <div style='width:54px;height:54px;flex-shrink:0;background:linear-gradient(135deg,#003344,#001e2e);
              border:1.5px solid {CY};border-radius:14px;display:flex;align-items:center;
              justify-content:center;font-size:26px;box-shadow:0 0 22px {CY}33;'>🏙️</div>
  <div>
  ...
  <div style='margin-left:auto;display:flex;gap:10px;flex-wrap:wrap;align-items:center;'>
    <span style='font-family:DM Mono,monospace;font-size:.66rem;letter-spacing:1.5px;
                 ...
                 border:1px solid {CY}55;color:{CY};'>MODEL · {model_choice.upper()}</span>
    <span style='font-family:DM Mono,monospace;font-size:.66rem;letter-spacing:1.5px;
                 padding:5px 14px;border-radius:6px;background:rgba(255,179,64,.08);
                 border:1px solid {AM}55;color:{AM};'>R² {mtr["r2"]:.4f}</span>
    <span style='font-family:DM Mono,monospace;font-size:.66rem;letter-spacing:1.5px;
                 padding:5px 14px;border-radius:6px;background:rgba(0,245,160,.08);
                 border:1px solid {GR}55;color:{GR};'>CONF {conf:.1f}%</span>
    <span style='font-family:DM Mono,monospace;font-size:.66rem;letter-spacing:1.5px;
                 padding:5px 14px;border-radius:6px;background:rgba(0,245,160,.10);
                 border:1px solid {GR}55;color:{GR};'>● LIVE</span>
  </div>
...
with T1:
    try:
        city_avg = (df.groupby("city")["stress_index"]
        ...
        for col,ico,lbl,val,sub,color in [
            ...
            (k3,"🚨","High-Stress Alerts",str(n_alerts),
             "Cities above 65 threshold",RD if n_alerts else GR),
            (k4,"🎯","Model Accuracy",f"{r2pct:.1f}%",f"R²={mtr['r2']:.4f}  MAE={mtr['mae']:.2f}",CY),
        ]:
            col.markdown(f"""
            <div class="kc"><div class="kc-ico">{ico}</div>
        ...
        mc,ml = stress_tier(most_s["ms"]), stress_tier(least_s["ms"])

        ch.markdown(f"""
        <div class="gc" style="border-color:{RD}33;">
          <div class="kc-lbl">🔴  Most Stressed City</div>
          <div style='font-family:Barlow Condensed,sans-serif;font-size:2rem;font-weight:800;
                      color:{RD};margin:6px 0;'>{most_s["city"]}</div>
          <div class="kc-val" style="font-size:1.8rem;color:{mc[0]};">{most_s["ms"]:.1f}</div>
          <div class="kc-sub">Mean Stress · {mc[1]} tier</div>
        </div>""", unsafe_allow_html=True)

        cl.markdown(f"""
        <div class="gc" style="border-color:{GR}33;">
          <div class="kc-lbl">🟢  Least Stressed City</div>
          <div style='font-family:Barlow Condensed,sans-serif;font-size:2rem;font-weight:800;
                      color:{GR};margin:6px 0;'>{least_s["city"]}</div>
        ...
        try:
            fig_r = go.Figure(go.Bar(
                x=city_avg["ms"], y=city_avg["city"], orientation="h",
                marker=dict(color=[stress_tier(v)[0] for v in city_avg["ms"]], line=dict(width=0)),
                text=[f"{v:.1f}" for v in city_avg["ms"]], textposition="inside",
        ...
        with tl:
            try:
                trend = (df.copy().assign(ym=df["date"].dt.to_period("M").astype(str))
                ...
                for i,c in enumerate(top4):
                    ...
                    fig_t.add_trace(go.Scatter(x=cd["ym"],y=cd["stress_index"],
                        mode="lines",name=c,line=dict(color=pal[i],width=2),
                        hovertemplate=f"<b>{c}</b><br>%{{x}}<br>%{{y:.1f}}<extra></extra>"))
                lay_t = plot_base("MONTHLY STRESS TRENDS · TOP 4 CITIES",h=300,yl="Stress Index",legend=True)
                lay_t["xaxis"]["tickangle"]=-40; lay_t["xaxis"]["nticks"]=8
        ...
        with tr:
            try:
                fig_v = go.Figure(go.Violin(
                    y=df["stress_index"], box_visible=True,
                    line_color=CY, fillcolor="rgba(0,229,255,0.07)", points="outliers",
        ...
        if n_alerts:
            ...
            for _,row in city_avg[city_avg["ms"]>65].iterrows():
                ...
                st.markdown(f"""
                <div class="ab" style="background:{bg};border:1px solid {sc2}44;color:{TX};">
                  <span style="font-size:1.1rem;">🚨</span>
                  <div><b style="color:{sc2};">{row["city"]}</b> — Mean Stress
                  <b style="color:{sc2};">{row["ms"]:.1f}</b> exceeds HIGH threshold.
                  <span style="color:{D2};font-size:.82rem;">Urban planning review recommended.</span></div>
                </div>""", unsafe_allow_html=True)
    ...
    try:
        ...
        with sl_col:
            ...
            for feat,m in FMETA.items():
                ...
                sv[feat] = st.slider(f"{m['ico']}  {m['lbl']}",
                    min_value=float(m["lo"]),max_value=float(m["hi"]),
                    value=d,step=float(m["step"]),
                    format=f"%.0f{m['unit']}",key=f"s_{feat}")
            st.markdown("<br>",unsafe_allow_html=True)
        ...
        with out_col:
            ...
            if score is None:
                ...
                <div style='background:rgba(0,229,255,.03);border:1px solid {BDR};
                  ...
                  <div style='font-family:Barlow,sans-serif;font-size:.84rem;color:{D3};margin-top:8px;'>
                ...
                try:
                    fig_g = go.Figure(go.Indicator(
                        ...
                        gauge=dict(
                            ...
                            steps=[{"range":[0,35],"color":"rgba(0,245,160,.07)"},
                                   {"range":[35,65],"color":"rgba(255,179,64,.07)"},
                                   {"range":[65,100],"color":"rgba(255,64,96,.07)"}],
                            threshold=dict(line=dict(color=sc_col2,width=3),thickness=.8,value=score),
                        ),
                    ))
                    fig_g.update_layout(paper_bgcolor="rgba(0,0,0,0)",
                        font=dict(family="Barlow Condensed"),height=250,
                ...
                <div style='text-align:center;margin:-6px 0 20px;'>
                  <div style='display:inline-flex;align-items:center;gap:10px;padding:10px 32px;
                              border-radius:8px;background:{sc_bg};border:1.5px solid {sc_col2}55;
                              font-family:Barlow Condensed,sans-serif;font-weight:800;
                              font-size:1.25rem;letter-spacing:3.5px;color:{sc_col2};text-transform:uppercase;'>
        ...
        if score is not None and inputs is not None:
            st.markdown('<div class="sl" style="margin-top:32px;">🔬  Scenario Comparison — Baseline vs Simulation</div>',
                        unsafe_allow_html=True)
            try:
                fl   = [FMETA[f]["lbl"] for f in FEATURE_COLS]
                bn   = [float(np.clip((city_meds.get(f,0)-FMETA[f]["lo"])/(FMETA[f]["hi"]-FMETA[f]["lo"]+1e-9),0,1)) for f in FEATURE_COLS]
                sn   = [float(np.clip((inputs.get(f,0)  -FMETA[f]["lo"])/(FMETA[f]["hi"]-FMETA[f]["lo"]+1e-9),0,1)) for f in FEATURE_COLS]
                fig_cmp = go.Figure()
                fig_cmp.add_trace(go.Bar(name=f"{ref_city} Baseline ({baseline:.1f})",
                    x=fl,y=bn,marker_color="rgba(0,229,255,0.45)",
                    marker_line=dict(color=CY,width=1),
                    hovertemplate="<b>%{x}</b><br>Baseline: %{y:.2f}<extra></extra>"))
                fig_cmp.add_trace(go.Bar(name=f"Simulation ({score:.1f})",
                    x=fl,y=sn,
                    marker_color=f"rgba({hex_rgb(sc_col2)},0.55)",
            ...
            # ── Sensitivity ───────────────────────────────────────────
            st.markdown('<div class="sl">📡  Sensitivity — Impact of ±20% Change</div>',
                        unsafe_allow_html=True)
            try:
                rows=[]
                for f in FEATURE_COLS:
                    uv=inputs.copy(); uv[f]=min(FMETA[f]["hi"],inputs[f]*1.2)
                    dv=inputs.copy(); dv[f]=max(FMETA[f]["lo"],inputs[f]*0.8)
                    rows.append({"Feature":FMETA[f]["lbl"],
                ...
                fig_s=go.Figure()
                fig_s.add_trace(go.Bar(name="+20%",x=sdf["Feature"],y=sdf["+20%"],
                    marker_color=[RD if v>0 else GR for v in sdf["+20%"]],opacity=.85,
                    marker_line=dict(width=0),
                    hovertemplate="<b>%{x}</b><br>+20%: %{y:+.2f}<extra></extra>"))
                fig_s.add_trace(go.Bar(name="-20%",x=sdf["Feature"],y=sdf["-20%"],
                    marker_color=[GR if v<0 else RD for v in sdf["-20%"]],opacity=.55,
                    marker_line=dict(width=0),
                    hovertemplate="<b>%{x}</b><br>-20%: %{y:+.2f}<extra></extra>"))
                lay_s=plot_base("SENSITIVITY ANALYSIS",h=280,yl="Δ Stress Index",legend=True)
                lay_s["barmode"]="group"
                lay_s["yaxis"]["zeroline"]=True
                lay_s["yaxis"]["zerolinecolor"]="rgba(0,229,255,.3)"
...
with T3:
    try:
        ...
        city_sel = st.selectbox("City",sorted(df["city"].unique()),index=0,label_visibility="collapsed")
        ...
        if cdf.empty:
            ...
            ck=st.columns(5)
            for col,ico,lbl,val,sub,color in [
                (ck[0],"📊","Mean Stress",f"{cdf['stress_index'].mean():.1f}",stress_tier(cdf['stress_index'].mean())[1],stress_tier(cdf['stress_index'].mean())[0]),
            ...
            try:
                fig_ts=go.Figure()
                fig_ts.add_trace(go.Scatter(x=cdf["date"],y=cdf["stress_index"],
                    mode="lines",name="Stress Index",line=dict(color=CY,width=2),
                    fill="tozeroy",fillcolor="rgba(0,229,255,0.06)",
                    hovertemplate="<b>Stress</b>: %{y:.1f}<br>%{x}<extra></extra>"))
                if "aqi" in cdf.columns:
                    fig_ts.add_trace(go.Scatter(x=cdf["date"],y=cdf["aqi"],
                        mode="lines",name="AQI",line=dict(color=AM,width=1.5,dash="dot"),
                        yaxis="y2",hovertemplate="<b>AQI</b>: %{y:.0f}<br>%{x}<extra></extra>"))
                lay_ts=plot_base(f"STRESS INDEX vs AQI · {city_sel.upper()}",h=320,
                                 yl="Stress Index",legend=True)
                lay_ts["yaxis2"]=dict(title=dict(text="AQI",font=dict(size=10,color=AM)),
            ...
            with bc:
                ...
                try:
                    ...
                    for m in mo:
                        ...
                        fig_b.add_trace(go.Box(y=md,name=m,marker_color=mc2,
                            line_color=mc2,fillcolor=f"{mc2}22",showlegend=False,
            ...
            with hc:
                ...
                try:
                    ...
                    for i in range(7):
                        if i not in piv.columns: piv[i]=0
                    piv=piv[[i for i in range(7)]]
                    mn=[calendar.month_abbr[m] for m in piv.index]
                    fig_h=go.Figure(go.Heatmap(
                        z=piv.values,
                        x=["Mon","Tue","Wed","Thu","Fri","Sat","Sun"],y=mn,
                        ...
                        colorbar=dict(tickfont=dict(family="DM Mono",size=9,color=D2),outlinewidth=0,thickness=10),
                    ))
            ...
            try:
                ...
                fig_c=go.Figure(go.Heatmap(
                    ...
                    colorbar=dict(tickfont=dict(family="DM Mono",size=9,color=D2),outlinewidth=0,thickness=11),
            ...
            try:
                ...
                lbc,vnc=lb+[lb[0]],fn+[fn[0]]
                fig_rad=go.Figure(go.Scatterpolar(
                    r=vnc,theta=lbc,fill="toself",fillcolor="rgba(0,229,255,.08)",
                    line=dict(color=CY,width=2),
                    hovertemplate="<b>%{theta}</b><br>%{r:.2f}<extra></extra>"))
                fig_rad.update_layout(
                    polar=dict(bgcolor="rgba(0,0,0,0)",
                        radialaxis=dict(visible=True,range=[0,1],
                            tickfont=dict(size=9,color=D3),gridcolor="rgba(0,229,255,.1)"),
                        angularaxis=dict(tickfont=dict(family="Barlow Condensed",size=12,color=TX),
                            gridcolor="rgba(0,229,255,.08)")),
                    paper_bgcolor="rgba(0,0,0,0)",font=dict(family="Barlow Condensed"),
                    title=dict(text=f"FEATURE PROFILE · {city_sel.upper()}",
                               font=dict(family="Barlow Condensed",size=15,color=TX),x=.5),
                    height=380,margin=dict(l=60,r=60,t=60,b=40))
...
with T4:
    try:
        ...
        <div style='background:rgba(0,229,255,.03);border:1px solid rgba(0,229,255,.14);
          ...
          <div style='font-family:Barlow Condensed,sans-serif;font-weight:800;
                      font-size:1.18rem;letter-spacing:2.5px;color:{CY};text-transform:uppercase;
                      margin-bottom:4px;'>🤖  AI Insight Engine</div>
        ...
        if score is None or inputs is None:
            ...
            # Score summary
            st.markdown(f"""
            <div class="gc" style="margin-bottom:24px;background:{sb3};border-color:{sc3}44;">
              <div style='display:flex;align-items:center;gap:18px;flex-wrap:wrap;'>
                <div>
                  <div style='font-family:DM Mono,monospace;font-size:.60rem;letter-spacing:2px;
                              text-transform:uppercase;color:{D3};margin-bottom:6px;'>Simulation Result</div>
                ...
                <div>
                  ...
                  <div style='font-family:Barlow,sans-serif;font-size:.84rem;color:{D2};margin-top:4px;'>
                    Model: {model_choice} · Confidence: {conf:.1f}%</div>
                </div>
              </div>
            </div>""",unsafe_allow_html=True)
            ...
            def make_insights(inp,sc_score):
                ...
                # AQI
                if v["aqi"]>=300:
                    rows.append(("🔴","CRITICAL",f"AQI {v['aqi']:.0f} — HAZARDOUS. Serious health risk for all populations. Emergency pollution reduction protocols required.",RD))
                elif v["aqi"]>=200:
                    rows.append(("🟠","HIGH RISK",f"AQI {v['aqi']:.0f} (Very Unhealthy). Industrial emission controls and vehicle access restrictions strongly recommended.",AM))
                elif v["aqi"]>=100:
                    rows.append(("🟡","MODERATE",f"AQI {v['aqi']:.0f}. Sensitive groups at risk. Low-emission zones and green transit investment advised.",AM))
                else:
                    rows.append(("🟢","GOOD",f"AQI {v['aqi']:.0f} within acceptable limits. Air quality is not a primary stressor.",GR))
                # Traffic
                if v["traffic_volume"]>=80:
                    rows.append(("🔴","HIGH CONGESTION",f"Traffic {v['traffic_volume']:.0f}/100 — severe gridlock. Congestion pricing and emergency PT expansion are urgent.",RD))
                elif v["traffic_volume"]>=55:
                    rows.append(("🟠","ELEVATED",f"Traffic {v['traffic_volume']:.0f}/100. Staggered work hours and ride-pooling incentives can reduce pressure.",AM))
                else:
                    rows.append(("🟢","OPTIMAL",f"Traffic {v['traffic_volume']:.0f}/100 is well-managed. Flow patterns are contributing positively.",GR))
                # PT
                if v["public_transport_usage"]>=70:
                    rows.append(("🟢","POSITIVE SIGNAL",f"PT usage {v['public_transport_usage']:.0f}/100 is excellent. High modal shift is the single most effective stress reducer.",GR))
                elif v["public_transport_usage"]>=45:
                    rows.append(("🟡","OPPORTUNITY",f"PT {v['public_transport_usage']:.0f}/100 — room to grow. Last-mile connectivity and fare subsidies could drive adoption.",AM))
                else:
                    rows.append(("🔴","UNDERUTILISED",f"PT critically low at {v['public_transport_usage']:.0f}/100. Private vehicle dominance magnifies all stressors.",RD))
                # Noise
                if v["noise_pollution"]>=75:
                    rows.append(("🔴","CRITICAL",f"Noise {v['noise_pollution']:.0f} dB far exceeds WHO threshold (55 dB). Noise barriers and construction curfews warranted.",RD))
                elif v["noise_pollution"]>=60:
                    rows.append(("🟡","ELEVATED",f"Noise {v['noise_pollution']:.0f} dB exceeds residential limits. Acoustic zoning recommended.",AM))
                # Temperature
                if v["temperature"]>=38:
                    rows.append(("🔴","HEAT ALERT",f"Temperature {v['temperature']:.0f}°C in danger zone. Cool roofs, tree canopy, and public cooling centres urgently needed.",RD))
                elif abs(v["temperature"]-22)>10:
                    rows.append(("🟡","THERMAL STRESS",f"Temperature {v['temperature']:.0f}°C deviates significantly from comfort zone (18–26°C). Physiological stress elevated.",AM))
                # Population
                if v["pop_density"]>=25000:
                    rows.append(("🔴","OVERCROWDING",f"Density {v['pop_density']:,.0f}/km² is extreme. Overcrowding compounds all stressors. Decentralisation imperative.",RD))
                return rows[:6]
            ...
            # Recommendations
            st.markdown('<div class="sl" style="margin-top:28px;">🎯  Strategic Recommendations</div>',
                        unsafe_allow_html=True)
            recs=[]
            if score>=65:
                recs.extend([
                    ("🚨","IMMEDIATE",RD,"Activate Emergency Urban Stress Protocol",
                    ...
                    ("🏛️","SHORT-TERM",AM,"Deploy Rapid Intervention Stack",
            ...
            for ri,rh,rc2,rt,rb_txt in recs[:5]:
                rb2=f"rgba({hex_rgb(rc2)},0.06)"
                st.markdown(f"""
                <div class="ab" style="background:{rb2};border:1px solid {rc2}33;margin-bottom:10px;">
                  <span style="font-size:1.2rem;">{ri}</span>
                  <div>
                    <div class="itag" style="background:{rb2};color:{rc2};border:1px solid {rc2}44;">{rh}</div>
                    <div style='font-family:Barlow Condensed,sans-serif;font-weight:700;
                                font-size:1rem;color:{TX};margin:4px 0 3px;'>{rt}</div>
                    <div style='font-family:Barlow,sans-serif;font-size:.83rem;color:{D2};line-height:1.5;'>
                      {rb_txt}</div>
                  </div>
                </div>""",unsafe_allow_html=True)

            # Summary table
            st.markdown('<div class="sl" style="margin-top:28px;">📊  Input Parameter Summary</div>',
                        unsafe_allow_html=True)
            try:
                srows=[]
                for f in FEATURE_COLS:
                    v=inputs[f]; lo=FMETA[f]["lo"]; hi=FMETA[f]["hi"]
                    pct=(v-lo)/(hi-lo+1e-9)*100
                    srows.append({"Feature":f"{FMETA[f]['ico']} {FMETA[f]['lbl']}",
                                  "Value":f"{v:.1f}{FMETA[f]['unit']}",
                                  "Scale %":f"{pct:.0f}%"})
...
with T5:
    try:
          ...
          <div style='display:flex;align-items:center;gap:16px;flex-wrap:wrap;'>
            ...
            <div>
              <div style='font-family:Barlow Condensed,sans-serif;font-weight:800;
                ...
                System Control & Model Panel</div>
              <div style='font-family:DM Mono,monospace;font-size:.62rem;letter-spacing:1.5px;
                          color:{D3};margin-top:5px;'>Session: {NOW_STR}</div>
            </div>
            <div style='margin-left:auto;display:flex;gap:10px;flex-wrap:wrap;'>
              {''.join(f"""
              <div style='padding:10px 20px;background:rgba({hex_rgb(c)},0.08);
                          border:1px solid {c}44;border-radius:10px;text-align:center;'>
                <div style='font-family:DM Mono,monospace;font-size:.55rem;letter-spacing:2px;
                            color:{D3};text-transform:uppercase;'>{lbl}</div>
                <div style='font-family:Barlow Condensed,sans-serif;font-weight:700;
                            color:{c};font-size:1rem;margin-top:3px;'>{val}</div>
              </div>""" for lbl,val,c in [
                ("Data Engine","● OPERATIONAL",GR),
                ("AI Engine","● ACTIVE",CY),
                (f"Predictions",pred_status,AM),
              ])}
            </div>
          </div>
        </div>""",unsafe_allow_html=True)
        ...
        with fi:
            ...
            try:
                ...
                fig_fi=go.Figure(go.Bar(
                    y=idf["f"],x=idf["i"],orientation="h",
                    marker=dict(color=bc2,line=dict(width=0)),
                    text=[f"{v*100:.1f}%" for v in idf["i"]],
                    textposition="outside",textfont=dict(family="DM Mono",size=10,color=D2),
        ...
        with avp:
            ...
            try:
                yt=mtr["yte"]; yp2=mtr["yp"]
                ns2=min(400,len(yt))
                idx=np.random.default_rng(0).choice(len(yt),ns2,replace=False)
                res=yp2[idx]-yt[idx]
                pc=[RD if abs(r)>10 else (AM if abs(r)>5 else GR) for r in res]
                fig_av=go.Figure()
                fig_av.add_trace(go.Scatter(x=[0,100],y=[0,100],mode="lines",
                    line=dict(color="rgba(0,229,255,.25)",width=1.5,dash="dash"),hoverinfo="skip"))
                fig_av.add_trace(go.Scatter(x=yt[idx],y=yp2[idx],mode="markers",
                ...
                lay_av["xaxis"]["range"]=[-2,103]; lay_av["yaxis"]["range"]=[-2,103]
        ...
        try:
            res2=mtr["yp"]-mtr["yte"]
            fig_res=go.Figure(go.Histogram(x=res2,nbinsx=50,
                marker=dict(color=CY,opacity=.55,line=dict(color=CY,width=.3)),
                hovertemplate="Residual: %{x:.1f}<br>Count: %{y}<extra></extra>"))
            fig_res.add_vline(x=0,line_color="rgba(0,229,255,.5)",line_width=1.5,line_dash="dash")
            lay_res=plot_base("RESIDUAL DISTRIBUTION (ŷ − y)",h=230,xl="Residual",yl="Frequency")
            lay_res["margin"]["t"]=40
        ...
        try:
            ...
            for c in sorted(df["city"].unique()):
                ...
                if len(cd2)<10: continue
                Xc=cd2[FEATURE_COLS].values; yc2=cd2["stress_index"].values
                yp3=model.predict(scaler.transform(Xc) if mtr["ns"] else Xc)
                ce.append({"City":c,"RMSE":round(np.sqrt(mean_squared_error(yc2,yp3)),2),
            ...
            fig_ce=go.Figure(go.Bar(
                ...
                marker=dict(color=[GR if v<4 else (AM if v<7 else RD) for v in ce_df["RMSE"]],
                            line=dict(width=0)),
                text=[f"{v}" for v in ce_df["RMSE"]],textposition="outside",
          ...
          <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;'>
            ...
            <div>
              <div style='font-family:DM Mono,monospace;font-size:.56rem;letter-spacing:2px;
                          text-transform:uppercase;color:{D3};'>{k}</div>
              <div style='font-family:Barlow,sans-serif;font-size:.86rem;color:{TX};margin-top:3px;'>{v}</div>
            </div>""" for k,v in [
              ("Algorithm",model_choice),("Features",f"{len(FEATURE_COLS)} urban indicators"),
              ("Train Split","80% time-based"),
              ("Scaler","StandardScaler" if mtr["ns"] else "None (tree model)"),
              ("Target","stress_index (0–100)"),("Session",NOW_STR),
            ])}
          </div>
        </div>""",unsafe_allow_html=True)
...
<div style='margin-top:52px;padding-top:18px;border-top:1px solid {BDR};
  ...
  <div style='display:flex;gap:20px;flex-wrap:wrap;'>
    <span style='font-family:DM Mono,monospace;font-size:.56rem;color:{D3};'>
      Data: OpenAQ · CPCB · IMD · Census India · UMTA</span>
    <span style='font-family:DM Mono,monospace;font-size:.56rem;color:{D3};'>
      Stack: Streamlit · scikit-learn · Plotly</span>
  </div>
</div>
""",unsafe_allow_html=True)
